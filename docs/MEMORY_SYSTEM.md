...
- accepted saved project templates after explicit confirmation
...
## Project Template Memory Boundary

Markdown project templates are durable only after the operator explicitly confirms a save from an
accepted proposal or creates a template directly. Before that point, detector output and proposal
review state are operational SQLite state, not markdown memory.

Accepted saved templates are creation-time seeds for future projects. They should capture reusable
shape, default tasks, prompts, or planning context, but they must not automatically rewrite existing
projects when a template is added or edited.

Proposal markdown should not be generated by the detector. The pure detector returns candidates only;
a later proposal state and filter service decides whether a candidate is eligible for Review Inbox,
and a still later explicit save flow may write canonical template markdown.
...
- provider payloads, tokens, encrypted values, or `secure_secrets`
...
- an accepted project-template proposal is explicitly confirmed for markdown save
