...
const OUTLOOK_MAIL_SCOPES = ["offline_access", "https://graph.microsoft.com/Mail.Read"];
const DEFAULT_REDIRECT_URI = "http://127.0.0.1:47843/oauth/outlook-email/callback";
const OUTLOOK_CALLBACK_SUCCESS_HTML =
  "<h1>Praxis connected Outlook Mail.</h1><p>You can close this browser tab.</p>";
const OUTLOOK_CALLBACK_ERROR_HTML =
  "<h1>Praxis could not connect Outlook Mail.</h1><p>Return to Praxis Desk for details.</p>";
...
const outlookEmailOAuthEvents = new EventEmitter();
...
const emitOutlookEmailOAuthUpdate = (update: OutlookEmailOAuthUpdate) => {
  outlookEmailOAuthEvents.emit("update", update);
};

export const onOutlookEmailOAuthUpdate = (listener: (update: OutlookEmailOAuthUpdate) => void) => {
  outlookEmailOAuthEvents.on("update", listener);
  return () => outlookEmailOAuthEvents.off("update", listener);
};

export const getOutlookEmailOAuthReadiness = (): EmailOAuthReadiness => {
  ...
  return {
    ...
    message:
      missing.length === 0
        ? "Outlook mail OAuth is configured enough to start the authorization-code flow."
        : `Outlook mail OAuth needs setup: ${missing.join(", ")}.`,
  };
};
...
const buildAuthorizationUrl = (
  ...
  url.searchParams.set("scope", OUTLOOK_MAIL_SCOPES.join(" "));
...
export const prepareOutlookEmailOAuth = async (
  input: StartEmailOAuthInput
): Promise<StartEmailOAuthResult> => {
  const readiness = getOutlookEmailOAuthReadiness();
  ...
  pendingOutlookOAuthStates.set(state, {
    state,
    codeVerifier,
