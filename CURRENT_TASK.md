...
Selector Phase 1 is complete in `praxis-desktop/shared/contextSurfaces.ts`. Checklist context groups and memory context lanes are pure shared selectors with regression coverage. UI consumption remains the next implementation slice.
...
Rainmeter snapshot polish is complete. The export keeps the existing nested fields and now adds skin-ready flat field names plus sanitized service-health label/detail copy for Rainmeter binding.

AI Review lane highlighting is complete. Suggested stable IDs from model or deterministic AI Task Review now carry into Today context-memory lanes and Checklist context groups as read-only visual highlights; model output still cannot create groups or write memory docs.

Service-health presence integration is complete. PRAXIS derives a public `attention` presence state from sanitized service health for the app header, tray tooltip, and Rainmeter snapshot while keeping persisted operator presence limited to `active`, `paused`, and `quiet_until`.

Notification boundary planning is complete in `docs/NOTIFICATION_BOUNDARY_PLAN.md`. Notification delivery remains unimplemented until Settings controls and quiet-window behavior are selected.

A simple Rainmeter skin draft now lives under `docs/rainmeter/Praxis/` and binds only to the read-only snapshot `skin` fields.

Notification delivery Settings controls are complete. Settings > Appearance now persists default-off desktop notification delivery plus a daily quiet window.

Rainmeter live work is paused until Rainmeter is installed on the operator machine.

Notification candidate selection, local snooze state, and click-through routing are complete. The app derives candidates for overdue work, waiting-on work, Review Inbox candidates, missed local planned blocks, and service attention; it filters them through notification settings, presence/quiet state, quiet-window state, and local snoozes before showing an in-app candidate with Open and Snooze actions. OS notification delivery remains the later step.
