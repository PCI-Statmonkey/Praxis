...
- Local model-path IPC QA now reaches structured model-ranked output with saved model `qwen2.5:0.5b-instruct`: reset, wins, forgetting, and risk return `summarySource: "ollama"` with deterministic packet-rendered copy, stale projects safely falls back when there are no stale IDs to rank, the write boundary stays read-only, and the work snapshot remains unchanged.
...
- Live Electron model-generated QA reached structured model-ranked output through IPC with `qwen2.5:0.5b-instruct`; actual Talk-surface QA should confirm visible model/fallback states after the JSON-mode contract.
...
- Live Electron QA against structured model-ranked AI Review output is complete or documented as remaining validation debt.
